...
uint8_t motor_get_state(motor_t *motor, uint8_t controller_state) {
    ...
    tab[1] = motor->address;
    tab[2] = 0x1F;
    tab[3] = controller_state;
    tab[4] = calculate_crc(buffer, 5);

    uart_send(motor, tab, 5);
    uart_receive(motor, tab, 5);

    if (tab[0] != 5 || tab[1] != 0xFF || tab[2] != 0x1F || calculate_crc(tab, 5) != tab[4]) {
        return 0xFF;
    }
...
void motor_set_angle(motor_t* motor, float angle){
	...
	int32_t pos = (int32_t)lroundf(angle * (131071.0f * 6.0f / 180.0f));

	motor_set_point(motor, pos);
}
...
static HAL_StatusTypeDef uart_send(motor_t *motor, uint8_t *data, uint16_t length) {
	HAL_GPIO_WritePin(motor->tx_en_port, motor->tx_en_pin, GPIO_PIN_SET);
	return HAL_UART_Transmit(motor->huart, data, length, UART_TIMEOUT);
}

static HAL_StatusTypeDef uart_receive(motor_t *motor, uint8_t *buffer, uint16_t length) {
	 HAL_GPIO_WritePin(motor->tx_en_port, motor->tx_en_pin, GPIO_PIN_RESET);
